// mye/scene/SceneSerializer.h — World ↔ JSON 씬 직렬화·저장/로드 (docs/03, 07 §3)
//
// World의 엔티티·컴포넌트(리플렉션)·계층(Parent/Children)·Entity 참조를 안정적으로 JSON에
// 왕복시킨다. 씬은 텍스트(JSON) 저장 권장(VCS diff). 04 리플렉션·직렬화를 소비.
//
// 위치 규약: 씬 직렬화는 ECS World↔JSON 이므로 scene 계층이 정본이다. 에디터(저장/Undo/프리팹),
//   게임 런타임(씬 로드), 세이브 시스템이 모두 이 한 구현을 공유한다.
//   (하위호환: mye::editor::SceneSerializer 는 이 타입의 별칭 — mye/editor/SceneSerializer.h.)
//
...
public:
    ...
    //   핸들이 없거나 재생성 실패 시 해당 엔티티는 Create() 로 폴백한다.
    //   씬 파일 로드(WriteWorld 결과)는 항상 preserveHandles=false(핸들 충돌 방지).
    //
    //   outLocalToEntity(선택): 로컬 ID → 새 Entity 매핑을 채워 반환한다.
